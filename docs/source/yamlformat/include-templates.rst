...
It is distributed by ``yaml2rpm`` RPM.  This template significantly reduces
each module yaml file that needs to be written.
...
Specific definitions and their use are described below.
Note that a few variables are initially defined in yaml packages files and
in the template they need to be overwritten as indicated.
